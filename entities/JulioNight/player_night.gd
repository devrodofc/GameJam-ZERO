...
# ==========================================
# CONTROLE DE ANIMAÇÕES VISUAIS
# ==========================================
func _update_animations() -> void:
	# Como pedido, focando apenas no movimento horizontal e idle por enquanto
...
func collect_memory(memory_id: String) -> void:
	if not collected_memories.has(memory_id):
		...
		# CHECAGEM: O jogador pegou as 3 memórias?
		if collected_memories.size() == 3:
			_show_final_memory_text()

func _show_final_memory_text() -> void:
	# Espera exatamente 3 segundos de forma assíncrona (sem travar o jogo)
	...
	# Adiciona a Label como filha do Player
	add_child(final_label)
	
	# Posiciona acima da cabeça do Player
	final_label.position = Vector2(0, -120.0)
	
	# Animação dramática: O texto sobe bem devagar e some depois de 4 segundos
